...
| 副作用安全 | 依赖 checkpoint 保存时机——若保存于副作用完成之前，恢复时可能重复执行 | **幂等**：确定性 callId 保证已完成的工具调用重放时不重新执行 |
...
- **库导出** ([src/index.ts](src/index.ts)) — 外部宿主（如 `@agent/coding-agent`）可 `import … from 'durable-agent-runtime'`：`Runtime`、`createHarnessWorkflow`、`ToolRegistry`、`callChat` 相关类型、`FileDeadLetterQueue`、`eval.ts` 全套打分器（`runEval`/`renderReport`/`Scenario`/scorers）等。Demo CLI 仍在本包 `bin`。
...
- **Eval 框架** ([src/eval.ts](src/eval.ts)) — **L3** 可组合打分器（结果性：`runCompleted`/`touchedFile`/`proposalContains`、`toolSuccessRate` 连续成功率、`costUnderUsd`；过程性：`turnsUnder`（回合预算）、`trajectoryJudge`（LLM 裁判工具调用序列而非只看最终答案）；人机协同：`humanInterventionRequested`/`humanInterventionsUnder`（读 `countingApprover` 产出的 `ApprovalStats`）；护栏回归：`noPolicyViolations`/`policyDenied`）+ runner，对派生出的 RunState / trace 打分；`agent eval` 发现回归时以非零退出码退出。整套打分器 + `runEval`/`renderReport` 已从 `index.ts` 导出，外部宿主可以拼自己的场景库——`@agent/coding-agent` 就是一个例子（见其 README「Eval」一节）。**单独评 harness 大脑**（assemble/compact/loop/scratchpad 等，不挂本 runtime）见 `@agent/harness` 的 L2 eval（[`docs/observability-trace-and-eval.md`](../docs/observability-trace-and-eval.md) §2.6）。
...
```bash
# 默认模式：每轮传递逐字 Q&A 对，零 LLM 开销
npm run dev -- chat

# 开启摘要模式：历史全量 messages 经 LLM 压缩为 system 摘要
# 最近 1 个 run 保持逐字，摘要结果跨轮缓存不复算
SESSION_HISTORY_MODE=full-summary npm run dev -- chat

# 摘要模式下，最近 run 保留全量 message transcript（含 tool calls/results）
SESSION_HISTORY_MODE=full-summary SESSION_VERBATIM_MODE=full-messages npm run dev -- chat
```
