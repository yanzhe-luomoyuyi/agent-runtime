...
- **文档 RAG / Retriever** ([src/retrieval/](src/retrieval/)) — 与 Memory **平行**：corpus 级文档 chunk 存储 + `Retriever` 缝 + `RetrievalPolicy`（默认 `mode: once` = query 进 loop 前系统检索一次，不默认让模型连搜）。`InMemoryDocumentStore` / `StoreRetriever` 同样可注入 `EmbeddingProvider`；hybrid 暴露 RRF 分；search 可选 `maxTextChars` 截断，全文走 `document_read`。
