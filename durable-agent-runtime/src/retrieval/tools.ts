/**
 * Document retrieval tools — durable ToolDefs over a DocumentStore.
 *
 * Register on the runtime ToolRegistry so every search/read flows through
 * `ctx.callTool` and is recorded for replay. Corpus binding is host-controlled
 * (model cannot invent ids outside the allow-list).
 *
...
/** Resolve model-supplied corpusId against the host allow-list (tool error strings). */
function resolveToolCorpusId(
  ...
  return [
    {
      ...
      run: async (args: unknown) => {
        ...
        const corpus = resolveToolCorpusId(b, a.corpusId);
