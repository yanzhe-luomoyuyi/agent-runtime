...
describe('observability trace', () => {
  ...
  it('builds a timeline with spans and token/cost totals', async () => {
    ...
    // writeFileMs is event-log disk time, stamped on RunCompleted.
    expect(trace.totals.writeFileMs).toBeGreaterThanOrEqual(0);
    expect(trace.totals.durableWrites).toBeGreaterThan(0);
  ...
  it('reads writeFileMs from RunCompleted, not tool spans', () => {
    const events: AgentEvent[] = [
      ...
      { type: 'RunCompleted', summary: {}, writeFileMs: 12, durableWrites: 4, ts: '2026-01-01T00:00:04.000Z' },
    ];
    const totals = buildTrace(events).totals;
    expect(totals.writeFileMs).toBe(12);
    expect(totals.durableWrites).toBe(4);
    expect(totals.toolMs).toBe(40);
  });
