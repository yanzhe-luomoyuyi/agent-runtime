...
| 层 | 改动 | 文件 |
|------|------|------|
| harness | `RunAgentOptions` 新增 `conversationHistory?: Message[]`；`initLoopState` 在 system prompt 和 goal 之间插入历史 | `agent-harness/src/control/loop.ts` |
| runtime | `RunInput` 新增 `conversationHistory`；`runtime.run()` 签名扩展；`makeContext` / harness-adapter / agent-loop 全线透传 | `types.ts` · `runtime.ts` · `workflow.ts` · `harness-adapter.ts` · `agent-loop.ts` |
| session | **新模块** `SessionManager`：JSON manifest 存储 session→runIds 映射；`start()` / `continue()` / `list()` / `get()` | `durable-agent-runtime/src/session.ts` |
| CLI | `agent chat` REPL + `--list` / `--history` / `--resume` 子命令 | `cli.ts` |
...
- **conversationHistory 只传 user↔assistant 公开对话** — 不传内部 tool-call turns（那会让上下文暴涨且泄露实现细节）
...
| 差距 | 说明 |
|------|------|
| 自动摘要旧对话 | 目前靠 ContextManager 的 token 硬顶裁剪，未做智能摘要 |
